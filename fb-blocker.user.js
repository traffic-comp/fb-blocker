...
// @downloadURL  https://github.com/traffic-comp/fb-blocker/raw/refs/heads/main/fb-blocker.user.js
// ==/UserScript==
...
(function() {
    ...
    class BlockedUsersAPI {
        async privateAsyncRequest(variables, docId, url = null, suppressEvaluation = true) {
    ...
    class UIManager {
        ...
        async createModal() {
            ...
            this.modal.querySelector("#fbblocked-export-btn").onclick = () => this.handleExport();
            this.modal.querySelector("#fbblocked-import-btn").onclick = () => this.handleImport();
            this.modal.querySelector("#fbblocked-stop-btn").onclick = () => this.cancelImport();
        ...
        async processImport(page, file) {
            ...
            const progressContainer = this.modal.querySelector("#fbblocked-progress-container");
            this.importCancelled = false;
            exportBtn.disabled = true;
            importBtn.disabled = true;
            ...
            try {
                ...
                for (let i = 0; i < batches.length; i++) {
                    if (this.importCancelled) {
                        this.log(`🛑 Import cancelled by user at batch ${i + 1}/${batches.length}`, "warning");
                        break;
                    }
                    const batch = batches[i];
                    const progress = ((i + 1) / batches.length) * 100;
                    this.updateProgress(progress, `Sending batch ${i + 1} of ${batches.length} (${batch.length} users)`);
                    this.log(`📤 Batch ${i + 1}/${batches.length} sent (${batch.length} users)`, "info");
                    try {
                        await this.api.blockUsers(page.id, page.access_token, batch);
                        processedUsers += batch.length;
                    } catch (error) {
                        this.log(`⚠️ Batch ${i + 1} API error (continuing anyway): ${error.message}`, "warning");
                        processedUsers += batch.length;
                    }
                    await this.sleep(500);
                }
                if (this.importCancelled) {
                    this.log(`Import stopped. Total users sent: ${processedUsers}`, "warning");
                } else {
                    this.log(`✅ Import complete! Total users sent: ${processedUsers}`, "info");
                }
            } catch (error) {
    ...
    // Железобетонная проверка наличия кнопки каждые 2 секунды (убивает SPA-проблему Фейсбука)
    setInterval(() => {
        if (!document.getElementById('fb-blocker-trigger')) {
            createTriggerButton();
        }
    }, 2000);
